...
public class LA32Ramp
{
    // SEMI-CONFIRMED from sample analysis.
    private const uint TARGET_SHIFTS = 18;
    private const uint MAX_CURRENT = 0xFF << (int)TARGET_SHIFTS;
    ...
    // scheme eventually.
    private const int INTERRUPT_TIME = 7;

    private static Bit16u[]? exp9;

    private Bit32u current;
    private uint largeTarget;
    private uint largeIncrement;
    ...
    public void StartRamp(Bit8u target, Bit8u increment)
    {
        ...
        else
        {
            ...
            Bit32u expArg = (uint)(increment & 0x7F);
            largeIncrement = (uint)(8191 - exp9![~(expArg << 6) & 511]);
            largeIncrement <<= (int)(expArg >> 3);
            largeIncrement += 64;
            largeIncrement >>= 9;
        }
        ...
        largeTarget = (uint)(target << (int)TARGET_SHIFTS);
        interruptCountdown = 0;
        interruptRaised = false;
    }
    ...
    public bool IsBelowCurrent(Bit8u target)
    {
        return (Bit32u)(target << (int)TARGET_SHIFTS) < current;
    }
}
